...
class AMASSDataset(Dataset):
    def __init__(self, mode, num_betas=16, batch_size=64, num_workers=12, **kwargs):
        ...
        self.template = bm.forward(root_orient=torch.tensor([[np.pi / 2, 0, 0]])).v.to(
            self.comp_device
        )
        ...
        if self.segm > 0:
            ...
            L, A = robust_laplacian.mesh_laplacian(
                template, template_f
            )  # SMPL Laplacian
            evals, evecs = sla.eigsh(
                L, self.segm, A, sigma=1e-8
            )  # Laplacian Eigendecomposition
            kmeans = KMeans(n_clusters=self.segm, random_state=0).fit(
                evecs
            )  # K-Means on eigenvectors
            self.labels = kmeans.labels_  # SAVE LABELS
            np.save(
                "segms_" + str(self.segm), self.labels
            )  # Save it locally to guarantee reproducibility
        else:
            self.labels = []

        # HOW MANY POINTS WE WANT TO MATCH FROM THE TEMPLATE?
        p = o3d.geometry.TriangleMesh(
            o3d.utility.Vector3dVector(np.squeeze(self.template.cpu().numpy())),
        ...
        # If factor is not set but n_points yes, use random n_points
            ...
            p = p.simplify_quadric_decimation(
                bm.f.shape[0] // self.factor
            )  # meaningful sampling
            kdt = KDTree(
                np.squeeze(self.template.cpu().numpy()), metric="euclidean"
            )  # Find indexs on the template
            self.idxs = np.squeeze(
                kdt.query(np.asarray(p.vertices), k=1, return_distance=False)
            )  # Store the idxs
            np.save(
                "idxs_" + str(self.factor), self.idxs
            )  # Save it locally to guarantee reproducibility
    ...
    def __getitem__(self, idx):
        ...
        if self.coeffs:
            ...
            data["occ"] = torch.reshape(
                (self.laplacian @ data["occ"]),
                (self.occ_res, self.occ_res, self.occ_res),
            )
        else:
            data["occ"] = torch.load(
                os.path.join(
        ...
        sample = {
            ...
            "idxs": self.idxs,
            "smpl_vertices_full": torch.as_tensor(
                data["data_v_scal"], dtype=torch.float32
            ),
            "smpl_inds": smpl_inds,
