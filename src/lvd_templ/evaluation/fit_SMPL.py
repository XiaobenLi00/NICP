...
def get_markers(args, inner_points, part_labels, confidences):
    ...
    valid_mask = torch.zeros(
        B, len(args.markerset), dtype=torch.bool, device=inner_points.device
    )
    pred_markers_position = torch.zeros(
        B, len(args.markerset), 3, device=inner_points.device
    )
    for b in range(B):
...
def fit_smpl(
    ...
    pred_markers_position, valid_mask = get_markers(
        args, inner_points, part_labels, confidences
    )  # return: shape(B, num_markers, 3) shape(B, num_markers)
    ...
    def marker_error_fn_0(optim_vars, aux_vars):
        ...
        marker_vindices = (
            torch.tensor(list(args.markerset.values()), device=smpl_vertices.device)
            .unsqueeze(0)
            .expand(batch_size, -1)
        )
