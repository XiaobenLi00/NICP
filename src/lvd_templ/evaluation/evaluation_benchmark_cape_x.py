...
from lvd_templ.paths import (
    chk_pts,
    home_dir,
    output_dir as out_folder,
    path_demo,
    path_demo_guess_rot,
)  # path_FAUST_train_reg, path_FAUST_train_scans
from lvd_templ.data.datamodule_AMASS import MetaData
from lvd_templ.evaluation.utils import (
    vox_scan,
    fit_LVD,
    selfsup_ref,
    SMPL_fitting,
...
def run(cfg: DictConfig) -> str:
    ...
    out_dir = out_folder + model_name + "/" + f"cape_{input_type}_x_cloth3d_100k_amass_120k_epoch_19"
    ...
    path_in = "output/tightness_vectors/cloth3d_tv_100k/eval_outputs_cape_vis/epoch_42_eval/vis"
    ...
    for scan in tqdm.tqdm(scans, desc="Scans:"):
        ...
        np.savez(
            out_dir + "/vis/" + name + "/pred_smplx_info_before_cham_refine.npz",
        ...
        if cfg["core"].cham_ref:
            ...
            np.savez(
                out_dir + "/vis/" + name + "/pred_smplx_info_after_cham_refine.npz",
                pose=params_np["pose"][:, 3:].reshape(-1, 3),
                betas=params_np["beta"].reshape(10),
                global_orient=params_np["pose"][:, :3].reshape(3),
                transl=params_np["trans"].reshape(3),
